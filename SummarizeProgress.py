...
class ProgressDashboard:
    ...
    def plot_pie(self):
        actual_runs = self.df[~self.df['Fastest_Mile'].isna()]['Miles_Ran'].sum()
        final_distance = self.df['Miles_Ran'].sum()
        remaining = final_distance - actual_runs
        
        data = [actual_runs, remaining]
        self.axs[0].pie(data, labels = ['Actual Mileage Ran', 'Total Training Miles'], explode = [0.1,0],
            colors = ['#44BBA4', '#393E41'], autopct= lambda p:f'{p*sum(data)/100 :.0f} Miles ({p:.2f}%)', radius = 1.5)
    ...
    def plot_pace(self):
        ...
        self.axs[1].set_ylim(5.5, 10.5)
        self.axs[1].set_title('Mile Pace Comparison Against Daily Goals')
        ticks = self.axs[1].get_xticks()
        self.axs[1].set_xticks([ticks[0], ticks[int(len(ticks)/2)], ticks[-1]], visible = True)
    ...
    def plot_goal_comparison(self):
        ...
        self.axs[3].set_ylim(-360,360)
        ticks = self.axs[3].get_xticks()
        self.axs[3].set_xticks([ticks[0], ticks[int(len(ticks)/2)], ticks[-1]], visible = True)
